...
import { TVShow, testData, Season } from "../types/TVShow";
...
//   });
//   const film = response.data.tvShow as Film;
...
const TVShowDetail: React.FunctionComponent<PageType> = (props) => {
  ...
  //     setTVShowData({ film, castAndCrew });
  ...
  //   try {
  //     const data = tvShowData as { film: Film; castAndCrew: CastAndCrew };
  //     await saveRatingAndReviewPromise(data.tvShow, rating, review);
  //     setShowModal(false);
  //     setReview(review);
  //     setRating(rating);
  //     setWatched(true);
  ...
  if (tvShowData === undefined) return null;
  return (
    <PageContainer className={`${styles.Container}`}>
      ...
      <div className={styles.PosterSection}>
        <Poster
          poster_path={tvShowData.tvShow.poster_path}
          rating={10}
        ></Poster>
      </div>
      ...
      <div className={styles.ButtonsContainer}>
        <Button
          ...
          onClick={() => {
            // if (!isToWatch) addFilmToList(tvShowData.tvShow, "toWatch");
            // else removeFilmFromList(tvShowData.tvShow.id, "toWatch");
          }}
        >
          To Watch
        </Button>
        <Button
          ...
          onClick={() => {
            // if (!isFavorite) addFilmToList(tvShowData.tvShow, "favorites");
            // else removeFilmFromList(tvShowData.tvShow.id, "favorites");
          }}
        >
          Favorite
        </Button>
